...
def summary(cur):
    total = utils.query_value(cur, 'SELECT COUNT(*) FROM swapi;')
    todo = utils.query_value(cur, 'SELECT COUNT(*) FROM swapi WHERE status IS NULL;')
    good = utils.query_value(cur, 'SELECT COUNT(*) FROM swapi WHERE status = 200;')
    error = utils.query_value(cur, 'SELECT COUNT(*) FROM swapi WHERE status != 200;')
...
count = utils.query_value(cur, sql)
...
while True:
    ...
    # Look through all of the "linked data" for other urls to retriee
